...
date: 04-10-2025      Revised 04-14-2025
output:
  github_document:
    toc: true
prerequisites:
...
-   How are farming occupation and islander worth related for the residents of Helvig?
    ...
    -   Null hypothesis: There is no statistical difference between the `Worth` in various farming `Type`s in Helvig.
...
To work to limit bias in the sampling plan (as most bias in observational studies is from sample) we are choosing to use a proportional stratified sample.
...
-   `Type`: This identifies the type of farmer. There are 5 different types of farmers: Dairy, Oat, Pig, Poultry, and Sheep.
    ...
    -   As we see from the table, there are 10 Dairy farmers, 11 Sheep farmers, 15 Poultry farmers, 25 Pig farmers, and 31 Oat farmers.
...
-   As illustrated by the ANOVA test there is no statistically significant difference in `Worth` between farmer `Type`. The F-value is 0.169 showing very little difference between group means compared to the variation within groups. When looking back at the null hypothesis ("There is no statistical difference between the `Worth` in various farming `Type`s in Helvig."), we fail to reject it. This is because when referencing the p-value of 0.953, there is no evidence of a (statistically) significant difference between `Type` and `Worth` between the different farming types on Helvig.
