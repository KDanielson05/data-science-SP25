...
df_q1 <-                     #assigning to f_q1
  flights %>%                #take the flights data set then
  select(ends_with("_time")) #select any column that ends with "time"
...
df_q2 <- 
  flights %>% 
  select(dest, origin, carrier, everything()) #dest, origin and carrier to front
df_q2
```
...
df_q3 <- 
  flights %>% 
  filter(between(arr_delay, -10, 10), arr_delay != 0)

df_q3

#lily said was right -- moved on
```
...
```{r}
flights %>% 
  mutate(arr_delay_comp = arr_time - sched_arr_time) %>% 
  select(arr_delay, arr_delay_comp, origin, dest) %>% 
  filter(arr_delay != arr_delay_comp)
```
...
**Observations** - Was this dataset assembled to study flights *out of* NYC, or *into* NYC? How do you know?

-   This was created to study flights going out of NYC as all of the data set is coming from NYC

    -   this highlights the fact that it was likely created for out of NYC, but does not prove it
