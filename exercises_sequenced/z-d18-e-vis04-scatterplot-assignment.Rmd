...
mpg %>% 
  ...
  geom_point( 
    data = mpg %>%             #this allows you to edit the dataset within graphing
      group_by(class) %>% 
...
mpg %>%
  ...
  geom_label_repel(                    #creates labels
    aes(label = class, fill = class),  #specifies label name and color
    data = df_annotate,                #tells which data will be referenced
    force_pull = 0.00001               #adjusts distance between data point and label
  )
```
...
-   the mileage of bwy tends to be larger – driving on the highway is more efficient

-   the trend of mileage with displ tends to be a decreasing linear trend as engine displacement increases – bigger cars tend to be less efficient
