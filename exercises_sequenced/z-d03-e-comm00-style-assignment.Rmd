...
diamonds %>%            #take the diamonds dataset then
  group_by(cut) %>%     #group by cut then
  summarize(            #summarize with mean price assigned mean_price
...
## TODO: Re-write the code above following the tidyverse style guide
iris %>%
  mutate(Sepal.Area = Sepal.Length * Sepal.Width) %>%  #creates a new column as a function of existing variables
  group_by(Species) %>%  
  summarize_if(is.numeric, mean) %>%  #summarize only if the mean is a number
  ungroup() %>%
  pivot_longer(names_to = "measure", values_to = "value", cols = -Species) %>%
  arrange(value)
```
...
```{r q3-task}
iris %>%                                 #take the iris then
  group_by(Species) %>%                  #group by species then
  summarize(                             #take means and assign titles
...
```{r q4-task}
iris %>%
  pivot_longer(                       #please explain pivot_longer, c() and \\.
    names_to = c("Part", ".value"),
    names_sep = "\\.",               
    cols = -Species
  ) %>%
