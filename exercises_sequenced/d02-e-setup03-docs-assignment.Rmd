...
iris %>%            #load iris then
  as_tibble() %>%   #convert data frame to a tibble then
  filter(           #choose rows only where the specues is setosa
    Species == "setosa"
  )
```
...
-   use function with no parentheses and the source code will show up
...
-   cut_number() is from ggplot2

-   parse_number() is from readr

-   row_number() is from dplyr
