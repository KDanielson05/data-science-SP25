...
Note that: - With `.value` in `names_to`, we do *not* provide the `values_to` column names. We are instead signaling that the value names come from the existing column names - `everything()` is a convenient way to select all columns
...
*Optional reading*: - [selection language](https://tidyselect.r-lib.org/reference/language.html)
