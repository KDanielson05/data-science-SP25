...
```{r}
str_c("a", "1")    #str_c joins multiple strings into one string
```
...
-   As ATL is an object, we would need to put it in quotes ("ATL") so that it could check
...
-   Continuous = smooth numbers.

    -   ex height – 5.8, 5.81, 5.812

-   Discrete = whole numbers or jumps.

    -   ex. how many pets you can have 1, 2, 3

-   Factors = categories/groups. Levels are the possible group names

    -   ex grades – levels A, B, C, D

    -   ex. variable shirt color – levels red, green, blue, etc
