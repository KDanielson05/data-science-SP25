...
flights |>
  summarize(across(
    where(is.numeric), # set of columns 
    # ~sum(is.na(.x))     # function to apply to each column
...
-   What is the mean departure delay time, among available values?
    -   The mean departure delay time among available values is 12.63907
...
-   Which carriers differ in the two lists?
    -   when sorted by mean, YV and EV appear while when sorting by median, UA and VX appear
...
test_iqr <-
  quantile(v_test_iqr, probs = 0.75) - quantile(v_test_iqr, probs = 0.25)
test_iqr
```
...
-   Which carriers are at the top of both `sd` and `IQR` departure delay?
    -   FL, F9, EV, YV
-   Which carriers differ in the two lists?
    -   on the sorted by sd list, there is HA
    -   on the sorted by IQR list, there is 9E
-   Which carrier has the biggest difference between its `sd` and `IQR`? What might this mean?
    -   HA had the biggest difference – this implies that there may be a large outlier
